...
```{r}
markers_used <- as.data.frame(unique(merged$MARKER_NM))
```

```{r}
final <- list()
for (s in 1:nrow(markers_used)) {
  marker_filter <- filter(merged, MARKER_NM == markers_used[s,])
  ...
  for (i in 1:nrow(traits_marker)) {
    ...
    test_df <- as.data.frame(c(traits_marker[i,], test[3]), col.names = c('TRAIT_NM', 'p-value'))
    test_df$MARKER_NM <- markers_used[s,]
  ...
  final[[s]] <- as.data.frame(bind_rows(out))
}
...
final_w <- list()
for (s in 1:nrow(markers_used)) {
  marker_filter_w <- filter(merged, MARKER_NM == markers_used[s,])
  ...
  out <- list()
  for (i in 1:nrow(traits_marker_w)) {
    trait_filter <- filter(marker_filter_w, TRAIT_NM == traits_marker_w[i,])
    line_AA <- filter(trait_filter, HAP_CALL == 'AA')
    line_BB <- filter(trait_filter, HAP_CALL == 'BB')
    if (is.data.frame(line_AA) && nrow(line_AA) <= 1) next
    if (is.data.frame(line_BB) && nrow(line_BB) <= 1) next
  ...
  final_w[[s]] <- as.data.frame(bind_rows(out))
}
...
for (t in 1:nrow(coop_loc)) {
  ...
  for (s in 1:nrow(loc_markers)) {
    ...
    for (i in 1:nrow(traits_marker)) {
      ...
      test_df$COOP_LOC_NM <- coop_loc[t,]
      test_df$TRAIT_NM <- as.character(test_df$TRAIT_NM)
      test_df$p.value <- as.double(test_df$p.value)
      test_df$MARKER_NM <- as.character(test_df$MARKER_NM)
  ...
  final[[t]] <- as.data.frame(bind_rows(out_2))
}
